...
3. Start the available trial with POST /api/v1/apps/{id}/billing/trial. If eligibility requires owner action, show the returned link. The source is free; hosting has a limited trial and paid plans. Do not claim perpetual free hosting or silently purchase a plan.
4. Poll app/jobs until hosting, database, auth and email are ready. Explain action_required responses. Apply schema.sql through POST /api/v1/apps/{id}/database/migrations with name "briefly_001" and the exact SQL content. Both languages use the same schema. Keep RLS and the auth.uid() policies.
5. POST /api/v1/apps/{id}/runtime with {}. This injects LB_RUNTIME_URL and LB_RUNTIME_TOKEN directly into hosting; it never returns the secret. The app-only token can send a fixed test email to a verified user of THIS app. It cannot manage infrastructure. Do not use your agent token instead.
...
7. Read https://launchbase.build/docs/deploy.md. Download https://launchbase.build/launchbase.mjs outside this folder. Run `node launchbase.mjs deploy ./briefly --app APP_ID --health /api/health --credential-file PRIVATE_FILE` using the credential saved by agent-connect.mjs (or LAUNCHBASE_TOKEN privately). Node.js 20+ runs the uploader; the app still runs Python. Do not serialize the app into a JSON file map. The helper packages real files and excludes secrets and dependencies. Exit 2: repeat the same command immediately to follow the same release. Exit 1: fix the reported build/runtime error and repeat. Confirm the detected health path is /api/health.
